...
// 用  途：数据编码
// 引数：纠错级别，型号（0 =自动)、型号自动扩展转折点，掩蔽号码(- 1 =自动)、编码数据，编码数据长度
// 返回值：编码成功时= true，没有数据，或超出容量时 = false
...
bool CQR_Encode::EncodeData(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource)
{
	...
	else
	{
		...
		{
			if (bAutoExtent)
				m_nVersion = nEncodeVersion; // 版本（型号）自动扩张
			else
				return false; // 超出容量
		}
	}
